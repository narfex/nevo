...
contract OracleHook is Ownable, IHooks {
    ...
    struct Token {
        ...
        bool isCustomReward;
        uint price; // Price in USD for fiats
        uint reward; // Referral percentage for fiats
        int commission; // Commission percentage (can be negative)
    ...
    function updatePrice(address _token, uint _price) external onlyUpdater {
        ...
        emit UpdatePrice(_token, _price);
    }
